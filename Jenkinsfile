...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3769
        NAMESPACE = "visual-vibe-uk"
        REGISTRY_HOSTNAME = "vibesamurai"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "visual-vibe-project"
        DEPLOYMENT_NAME = "visual-vibe-project-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'visual-vibe-uk') {
